...
var points = document.getElementsByClassName('scorePoints')[0]; //показать счет

function init() {
    prepareGameField(); // Генерация поля
    points.innerHTML = score; // присваиваем значение текущего счета
...
function startGame() {
    ...
    // setTimeout(createBarrier, 12000);
    setInterval(createBarrier, 12000);
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && isNotBarrier(new_unit)) {
